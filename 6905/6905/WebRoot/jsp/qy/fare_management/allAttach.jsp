					...
					<div class="file-table-box">
						<table>
							...
							<tbody>
								...
								<tr>
									<td><%=realName%></td>
									<td class="file-list-box-td-small"><a
										href="FareServlet?operate=download&curPageNum=<%=curPageNum%>&pageSize=<%=pageSize%>&builtType=<%=builtType%>&startTime=<%=startTime%>&endTime=<%=endTime%>&storeCompany=<%=storeCompany%>&realName=<%=realName%>&path=<%=attach.getAttachPath()%>&fareId=<%=attach.getAttachId()%>"
										class="download-btn">下载 </a> <input type="button" value="删除"
										onclick="deleteAttach('FareServlet?curPageNum=<%=curPageNum%>&pageSize=<%=pageSize%>&builtType=<%=builtType%>&startTime=<%=startTime%>&endTime=<%=endTime%>&storeCompany=<%=storeCompany%>&operate=deleteAttach&attachId=<%=attach.getAttachId()%>&fareId=<%=fareId%>')">
									</td>
								</tr>
								<%
									}
