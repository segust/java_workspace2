		...
		<div id="content">
			...
			<%
				//curPageNum 当前页
																...
																//如果添加成功,或者是编辑成功后，就获的该条记录的fareId 
															String fareId="";
															if(request.getParameter("fareId")!=null){
															fareId=request.getParameter("fareId"); 
															}
			%>
			<input type="hidden" id="lastFareId" value='<%=fareId%>'>
			<div class="fare-data" id="fare-data">
				...
				<form name="checkForm" method="post">
					<div id="search-box">
						<p>
							费用类型 <input type="hidden" name="builtType"> 
							<input type="checkbox" id="box" onclick="allSelect()"> 
							<!-- <label
								for="1"> 器材购置费 </label> <input id="1" name="fareType"
								type="checkbox" value="器材购置费">  -->
							<label for="2">
								运杂费 </label> <input id="2" name="fareType" type="checkbox" value="运杂费">
							<label for="3"> 轮换费 </label> <input id="3" name="fareType"
								type="checkbox" value="轮换费"> <label for="6">
								维护保养费 </label> <input id="6" name="fareType" type="checkbox"
								value="维护保养费"> <label for="8"> 其他 </label> <input id="8"
								name="fareType" type="checkbox" value="其他"> <input
								class="add-btn" type="button"
								onclick="window.location.href='jsp/qy/fare_management/addFare.jsp?sum=<%=sum%>&curPageNum=<%=curPageNum%>&totalPageNum=<%=totalPageNum%>&pageSize=<%=pageSize%>'"
								value="添加经费"> <input id="export-btn" type="button"
								disabled="disabled" class="unable-export-btn"
								onclick="selectId('export')" value="导出经费">
						</p>
						<p>
							时间段：<input type="text" id="startDate" class="sang_Calender"
								name="startTime" /> 到 <input type="text" id="endDate"
								class="sang_Calender" name="endTime" />
							<script type="text/javascript"
								src="ConstantHTML/js/spdateTime.js"></script>
							<input type="hidden" value="" name="storeCompany"> <span
								...
								class="search-btn" type="button"
								onclick="checkInput('statisticsDetail')" value="明细统计"> </span>
						</p>
					</div>
				</form>
				<div id="fare-table-box">
					<form name="exportForm" method="post">
						<table id="fare-table" class="fare-table">
							<thead>
								<tr>
									<th class="fare-table-checkbox"><input id="allCheckbox"
							...
							<tbody id="checkfare-tbody">
								<%
									String startTime=(String)request.getAttribute("startTime");
																					                        ...
																					                        String newStr = builtType.replaceAll("'","");  
																										  	...
																										  	FareDetailService fs=new FareDetailService();
								...
								<tr>
									...
									<td><input type="button" value="查看明细"
										...
										onclick='window.location.href="jsp/qy/fare_management/editFare.jsp?curPageNum=<%=curPageNum%>&pageSize=<%=pageSize%>&builtType=<%=newStr%>&startTime=<%=startTime%>&endTime=<%=endTime%>&storeCompany=<%=storeCompany%>&fareId=<%=fare.getFareId()%>" '>
										<input type="button" value="删除"
										onclick="delAFare('FareServlet?operate=delete&curPageNum=<%=curPageNum%>&pageSize=<%=pageSize%>&builtType=<%=newStr%>&startTime=<%=startTime%>&endTime=<%=endTime%>&storeCompany=<%=storeCompany%>&fareId=<%=fare.getFareId()%>')">
										<input type="button" value="附件"
										onclick="window.location.href='/6905/FareServlet?curPageNum=<%=curPageNum%>&pageSize=<%=pageSize%>&builtType=<%=newStr%>&startTime=<%=startTime%>&endTime=<%=endTime%>&storeCompany=<%=storeCompany%>&operate=attach&fareId=<%=fare.getFareId()%>'">
									</td>
								</tr>
								<%
									} 
					...
					<form id="idForm" name="idForm" method="post">
						<input type="hidden" id="idStr" name="idStr1" value=""> <input
							type="hidden" id="noSelect" name="noSelect1" value="">
					</form>
				</div>
				<!-- 分页的盒子 -->
				<div class="page-box">
...
function selectId(text){
	 ...
	 case "jump"://跳页  <!-- 跳转到多少页时 js的skipPage()函数先判断输入的页码是否有效-->
	...
	<!-- 删除费用 -->
	<script>
		function delAFare(url){
			window.wxc.xcConfirm("确认删除吗？","confirm",{onOk:function(){ 
				window.wxc.xcConfirm("删除成功!","success",{onOk:function(){
