	...
	<body>
		...
		<div id="right">
			...
			<div class="fare-data" id="fare-data">
				<div class="search_box">
					<form action="EquipmentServlet?operate=equipmentDetailAccount"
						...
						<span>产品名称</span>
						<span><input class="setTem" type="text" name="productName" />
						<span>产品型号</span>
						...
						<input type="button" value="导出" class="scan-btn"
							onclick="equipmentDetailAccountToWord();">
					</form>
				</div>
				<div id="whole"></div>
				<div id="mingxitable">
					...
					<table id="equipment_detail_account">
	                    ...
	                    %>
							<tr class="contract-add-table-body">
								<td>
									<input type="checkbox" name="checkbox_strength" />
								</td>
								<td><%=i+pageNumber+1 %></td>
								 <td><%=T.get(i).get("productName").toString() %>+<%=T.get(i).get("productModel").toString() %></td>
		                          ...
		                          <td><input type="text" ></td>
								<td>
									<input type="button" value="明细账" name="operate_detail"
			...
			<div class="page-box">
				...
				%>
				<span> <a
					href="EquipmentServlet?operate=equipmentDetailAccount&curPageNum=1&pageSize=<%=pageSize%>&inYear=<%=inYear %>&QCBM=<%=QCBM%>&productModel=<%=productModel%>&productName=<%=productName%>&keeper=<%=keeper%>">首页</a>
					<a
					href="EquipmentServlet?operate=equipmentDetailAccount&curPageNum=<%=prePageNum %>&pageSize=<%=pageSize %>&inYear=<%=inYear %>&QCBM=<%=QCBM%>&productModel=<%=productModel%>&productName=<%=productName%>&keeper=<%=keeper%>">&lt;&lt;</a>
					<span><%=curPageNum %></span> <a
					href="EquipmentServlet?operate=equipmentDetailAccount&curPageNum=<%=nextPageNum %>&pageSize=<%=pageSize %>&inYear=<%=inYear %>&QCBM=<%=QCBM%>&productModel=<%=productModel%>&productName=<%=productName%>&keeper=<%=keeper%>">&gt;&gt;</a>
					<a
					href="EquipmentServlet?operate=equipmentDetailAccount&curPageNum=<%=totalPageNum %>&pageSize=<%=pageSize %>&inYear=<%=inYear %>&QCBM=<%=QCBM%>&productModel=<%=productModel%>&productName=<%=productName%>&keeper=<%=keeper%>">尾页</a>
				</span>
				<span>跳到第</span>
