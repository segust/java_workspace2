...
function checkPass(type, check,event) {
	...
	if (flag) {
		...
		msg
				.setAttribute("style",
						"color: red;width: 200px;margin-left: auto;margin-right: auto;");
	}
}
// 获取前端数据并传到后台
//isOneclick判断是否为一键审核，1为是，0为不是
function synDatabase(type, check, array, ownedUnit,isOneClick) {
	jQuery
			.ajax({
				type : "post",
				url : "InWarehouseServlet?operate=checkFileSynchroInJDS",
				data : {
				...
				success : function(data) {
					if (data == "保存成功！") {
						...
						msg
						.setAttribute("style",
								"color: blue;width: 200px;margin-left: auto;margin-right: auto;");
						var username = getEleListByClassName("info_user",document.body);
		...
		if (oCurrentCheckBox[i].checked) {
			...
			for (var j = 0; j < 7; j++) {
				if (j == 0) {
					...
					if(exportType == "circleOut" || exportType=="renewOut") {
						ownedUnit = oCurrentTd[j+1].innerHTML;
					}else
						ownedUnit = oCurrentTd[j].innerHTML;
				}
			}
...
function synUpdateDatabase(type) {
	...
	for (var i = 1; i < trs.length; i++) {
		...
		for (var j = 0; j < tds.length; j++) {
			if (j == 0) {
				...
				var user = tds[j].getElementsByTagName("p")[0].innerHTML;
				o.push(user);
			} else {
	...
	jQuery
			.ajax({
				type : "post",
				url : "InWarehouseServlet?operate=checkFileSynchroInJDS&applyType="
						+ type,
				data : {
					'data' : arrayToJson(array)
				},
