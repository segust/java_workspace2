...
//获取前端数据并传到后台
//isOneClick 标志是否为一键通过
function synDatabase(type,check,array,ownedUnit,isOneClick) {
	jQuery.ajax({
		type : "post",
		url : "InWarehouseServlet?operate=checkFileInZHJ&applyType="+type+"&ownedUnit="+ownedUnit,
		data : {
...
function downloadAll(type) {
	...
	if(flag){
		jQuery.ajax({
			type : "post",
			url : "InWarehouseServlet?operate=exportSingleForm&exportType="+exportType+"&ownedUnit="+ownedUnit,
			data : {
