                ...
                switch (evt.keyCode) {
                    ...
                    case keys.enter:
                        ...
                        if(id.toLowerCase() == "pmnm") {
                        	...
                        	goForUnitInfo(val);
                        }else if (id.toLowerCase() == "deviceno") {
                        	goForProduct(val);
                        }
                ...
                if(id.toLowerCase() == "pmnm") {
                	...
                	var wholename = document.getElementById("wholename").value;
                     if (v.size()) {
                         ...
                         oldVal = v.text();
                         //靠上下键盘触发离开
                         goForDeviceNo(pmnm,wholename,oldVal);
                         goForUnitInfo(oldVal);
                         hide();
                     }
                }else if(id.toLowerCase() == "wholename") {
                	var pmnm = document.getElementById("pmnm").value;
                	var unit = document.getElementById("productuint").value;
                     if (v.size()) {
                         textBox.val(v.text());
                         oldVal = v.text();
                         //靠上下键盘触发离开
	...
	$.ajax({
		...
		success : function(data) {
			if(data != "") {
				...
				suggestion += "]";
				$('.dNo').combobox(suggestion.replace('[','').replace(']','').split(','));
			}
		},
		error : function() {
			alert("请求失败");
...
function goForUnitInfo(unitName) {
	$.ajax({
		type : "post",
		url : "ProductHandleServlet?operate=goForUnit&unitName="+unitName,
		dataType : "text",
		success : function(data) {
			if(data != "") {
...
function goForProduct(deviceNo) {
	if(deviceNo != "") {
		$.ajax({
			type : "post",
			...
			success : function(data) {
				...
				if(data != "") {
					...
					manuf.setAttribute("value", product.manuf);
