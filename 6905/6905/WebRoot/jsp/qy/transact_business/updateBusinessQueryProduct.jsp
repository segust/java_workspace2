      ...
      <%@include file="../../../ConstantHTML/left.html" %>
       ...
       <div id="content">
        ...
        <div id="lunhuanchurukuguanli">
            ...
            <form method="post" action="BorrowServlet?operate=updateInOut">
				<span>合同号</span> 
       	...
       	<div class="page-box">
				...
				%>
			    <span>
			    <a href="BorrowServlet?operate=updateInOut&P.contractid=<%=condition.get("P.contractId") %>&productmodel=<%=condition.get("productModel") %>&unitname=<%=condition.get("productUnit") %>&signdate=<%=condition.get("signDate") %>&productName=<%=condition.get("productName") %>&curPageNum=1&pageSize=<%=pageSize %>" >首页</a>
			    <a href="BorrowServlet?operate=updateInOut&P.contractid=<%=condition.get("P.contractId") %>&productmodel=<%=condition.get("productModel") %>&unitname=<%=condition.get("productUnit") %>&signdate=<%=condition.get("signDate") %>&productName=<%=condition.get("productName") %>&curPageNum=<%=prePageNum %>&pageSize=<%=pageSize %>" >&lt;&lt;</a>
			   	<span><%=curPageNum %></span>
			    <a href="BorrowServlet?operate=updateInOut&P.contractid=<%=condition.get("P.contractId") %>&productmodel=<%=condition.get("productModel") %>&unitname=<%=condition.get("productUnit") %>&signdate=<%=condition.get("signDate") %>&productName=<%=condition.get("productName") %>&curPageNum=<%=nextPageNum %>&pageSize=<%=pageSize %>">&gt;&gt;</a>
			    <a href="BorrowServlet?operate=updateInOut&P.contractid=<%=condition.get("P.contractId") %>&productmodel=<%=condition.get("productModel") %>&unitname=<%=condition.get("productUnit") %>&signdate=<%=condition.get("signDate") %>&productName=<%=condition.get("productName") %>&curPageNum=<%=totalPageNum %>&pageSize=<%=pageSize %>">尾页</a>
			    </span>
		    	<span>跳到第</span>
		...
		<script type="text/javascript">
			function skipPage(){
			 	...
			 	if(skipPageNum<=0)
					...
					window.location.href="BorrowServlet?operate=updateInOut&P.contractid=<%=condition.get("P.contractId") %>&productmodel=<%=condition.get("productModel") %>&unitname=<%=condition.get("productUnit") %>&signdate=<%=condition.get("signDate") %>&productName=<%=condition.get("productName") %>&curPageNum="+skipPageNum+"&pageSize="+pageSize;
			}
		</script>
		...
		<script type="text/javascript">
			function selectPageSize(value){
				var pageSize=value;
				window.location.href="BorrowServlet?operate=updateInOut&P.contractid=<%=condition.get("P.contractId") %>&productmodel=<%=condition.get("productModel") %>&unitname=<%=condition.get("productUnit") %>&signdate=<%=condition.get("signDate") %>&productName=<%=condition.get("productName") %>&curPageNum=1&pageSize="+value;
			}
		</script>
