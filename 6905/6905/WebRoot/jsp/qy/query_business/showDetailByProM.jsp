	...
	<%
	 ...
	 %>
		...
		<div id="right">
					...
					<table class="fare-table" id="fare-table">
						...
						<tbody>
							...
							%>
								...
								<td><%=recordList.get(i).get("operateType1")%></td>
								...
								<td><%=recordList.get(i).get("operateType2")%></td>
			...
			<div class="page-box">
					<span> <a
						href="UpdateQueryServlet?operate=updateQueryDetailByProductModel&curPageNum=1&pageSize=<%=pageSize %>&productModel=<%=productModel %>">首页</a>
						<a
						href="UpdateQueryServlet?operate=updateQueryDetailByProductModel&curPageNum=<%=prePageNum %>&pageSize=<%=pageSize %>&productModel=<%=productModel %>">&lt;&lt;</a>
						<span><font color="black"><%=curPageNum %></font>
					</span> <a
						href="UpdateQueryServlet?operate=updateQueryDetailByProductModel&curPageNum=<%=nextPageNum %>&pageSize=<%=pageSize %>&productModel=<%=productModel %>">&gt;&gt;</a>
						<a
						href="UpdateQueryServlet?operate=updateQueryDetailByProductModel&curPageNum=<%=totalPageNum %>&pageSize=<%=pageSize %>&productModel=<%=productModel %>">尾页</a>
					</span>
