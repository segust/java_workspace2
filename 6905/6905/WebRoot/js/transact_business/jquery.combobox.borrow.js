...
function goForProduct(deviceNo) {
	if(deviceNo !="") {
		$.ajax({
			type : "post",
			url : "ProductHandleServlet?operate=findByNo&deviceNo="+deviceNo+"&isIn="+0,
			dataType : "text",
			success : function(data) {
				if(data != "") {
					var product = eval("("+data+")");
					var manu = document.getElementById("manufacturer");
					manu.setAttribute("value", product.manuf);
					...
					var pDate = document.getElementById("signdate");
					...
					wholename.setAttribute("value", product.wholename);
					var productunit = document.getElementById("productunit");
					productunit.setAttribute("value", product.unitName);
