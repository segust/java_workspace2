        ...
        <div id="lunhuanchurukuguanli" style="margin-top: 5px;">
            ...
            <div class="search-box"  >
				<form method="post"
						...
						<span style="color: red;">请选择军代室：</span>
							<select name="JDS" id="JDS" >
								<option value="all">请选择</option>
								<c:forEach var="JDS" items="${JDSes }">
								<option value="${JDS}">${JDS}</option>
							</c:forEach>
						</select>
						<span><input class="search-btn" type="submit" value="查询申请" style="background-color: #3CB371;"></span>
					</form>
            </div>
            <div id="whole"></div>
            <div id="borrow_In">
            ...
            <div id="new_In">
				   ...
				   <table id="contract-table">
						...
						%>
							<tr class="contract-add-table-body">
									<td><input type="checkbox" name="mycheck" onchange="checkChange(this)"/>
									...
									<td><input input type="button" value="查看" name="operate_detail"
					...
					<input type="button" class="search-btn" value="导出" onclick="checktransact('<%=request.getSession().getAttribute("ownedUnit") %>');">
				</div>
			</div>
		</div>
		</div>
		  	<div class="page-box">
				...
				<%
					...
					%>
			    <span>
			    <a href="InWarehouseServlet?operate=queryApply&curPageNum=1&pageSize=<%=pageSize %>" >首页</a>
			    <a href="InWarehouseServlet?operate=queryApply&curPageNum=<%=prePageNum %>&pageSize=<%=pageSize %>" >&lt;&lt;</a>
			   	<span><%=curPageNum %></span>
			    <a href="InWarehouseServlet?operate=queryApply&curPageNum=<%=nextPageNum %>&pageSize=<%=pageSize %>">&gt;&gt;</a>
			    <a href="InWarehouseServlet?operate=queryApply&curPageNum=<%=totalPageNum %>&pageSize=<%=pageSize %>">尾页</a>
			    </span>
