...
<body>
	...
	<div id="right">
		...
		<div class="subName">
			<span>当前位置：</span> <a
				href="EquipmentServlet?operate=equipmentCollective&curPageNum=1&pageSize=10">实力统计</a>&nbsp;&nbsp;>&nbsp;&nbsp;
		...
		<div class="fare-data" id="fare-data">
			<div class="search_box">
				<form action="EquipmentServlet?operate=equipmentDetailAccount"
					method="post" onsubmit="return judgeInputTime('inYearId')">
					<span style="margin-left:10px;">在库年份</span> <span><input
						class="setTem" type="text" name="inYear" id="inYearId" />
					</span> 
					<span>器材代码</span> 
					<span><input class="setTem" type="text"
						name="QCBM" /></span>
					<span>产品名称</span>
						...
						<span>代储企业</span>
							<select name="keeper" id="keeper" >
								<option value="all">全部代储企业</option>
								<%
									String ownedUnit=(String)request.getSession().getAttribute("ownedUnit");
									List<String> companyList= new InfoService().getCompanyNameList(ownedUnit, 2);
									for(int i=0;i<companyList.size();i++){
								 %>
								<option value="<%=companyList.get(i) %>"><%=companyList.get(i) %></option>
								<%
								} 
								%>
						</select>
						<span> <input class="scan-btn" type="submit" value="查询">
						<input type="button" value="导出" class="scan-btn"
						onclick="equipmentDetailAccountToWord();"> </span>
				</form>
			</div>
			<div id="whole"></div>
			<div id="mingxitable">
			...
			<div id="equipment_detail_account-box">
				...
				<table id="equipment_detail_account">
					<thead>
						<tr>
							<th><input type="checkbox" id="checkboxLeader"
                    ...
                    %>
						<tr class="contract-add-table-body">
							...
							<td><input type="text"></td>
							<td><input type="text"></td>
							<td><input type="button" value="明细账" name="operate_detail"
								onclick="out_EquipmentDetailAccount(this)">
							</td>
						</tr>
						<%} %>
					</tbody>
		...
		<div class="page-box">
			<% 
				...
				%>
			<span> <a
				href="EquipmentServlet?operate=equipmentDetailAccount&curPageNum=1&pageSize=<%=pageSize%>&inYear=<%=inYear %>&QCBM=<%=QCBM%>&productName=<%=productName%>&productName=<%=productName%>&keeper=<%=keeper%>">首页</a>
				<a
				href="EquipmentServlet?operate=equipmentDetailAccount&curPageNum=<%=prePageNum %>&pageSize=<%=pageSize %>&inYear=<%=inYear %>&QCBM=<%=QCBM%>&productName=<%=productName%>&productName=<%=productName%>&keeper=<%=keeper%>">&lt;&lt;</a>
				<span><%=curPageNum %></span> <a
				href="EquipmentServlet?operate=equipmentDetailAccount&curPageNum=<%=nextPageNum %>&pageSize=<%=pageSize %>&inYear=<%=inYear %>&QCBM=<%=QCBM%>&productName=<%=productName%>&productName=<%=productName%>&keeper=<%=keeper%>">&gt;&gt;</a>
				<a
				href="EquipmentServlet?operate=equipmentDetailAccount&curPageNum=<%=totalPageNum %>&pageSize=<%=pageSize %>&inYear=<%=inYear %>&QCBM=<%=QCBM%>&productName=<%=productName%>&productName=<%=productName%>&keeper=<%=keeper%>">尾页</a>
