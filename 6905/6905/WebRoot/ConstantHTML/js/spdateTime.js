(function() {
	var d = document, w = window, isIE = w.navigator.appVersion.indexOf("MSIE") > -1, now = new Date(), nowM = now
			.getMonth();
	nowY = now.getFullYear();
	var date = null, ids = null, oInput = null;
	document
			.write('<iframe frameborder=0 style="display:none;position:absolute;" width="200" height="215" scrolling="no" name="sangcalender" id="sangcalender" ></iframe>');
	var f = window.frames['sangcalender'];
	var ff = d.getElementById('sangcalender');
	var fd = f.document;
	fd.open();
	fd
			.write('<!DOCTYPE html><html><head><style type="text/css">#yearL, #monthL, #monthR, #yearR, #hoursL, #hoursR, #minL, #minR, #y, #m, #h, #i, #s{cursor:pointer;}.calenderClose a{display:block;width:13px;line-height:13px;border:1px solid #cccccc;background-color:#eeeeee;color:#666; text-decoration:none}.calenderClose a:hover{color:red}td{text-align:center}#calenderDay{cursor:pointer}body{font-size:12px;padding:0;margin:0}.col666{color:#999}.bg9ebdd6{background-color:#66FF99}</style></head><body onselectstart="return false" style="-moz-user-select:none" oncontextmenu="return false">');
	fd
			.write('<table width="100%" border="0" bgcolor="#CCCCCC" cellspacing="1" cellpadding="0">'
					+ '<tr><td><table border="0" cellspacing="0" bgcolor="#33CC33" cellpadding="0" width="100%">'
					...
					+ '<tr bgcolor="#FFFFFF"><td><table width="100%" border="0" bgcolor="#8F8C8F" cellspacing="1" cellpadding="0">'
					+ '<tr bgcolor="#66CC66" height="18"><td>日</td><td>一</td><td>二</td><td>三</td><td>四</td><td>五</td><td>六</td></tr></table></td></tr>'
	...
	function formatDay(timestr) {
		...
		date = getdate(da);
		var mon = [ 31, date['y'] % 4 == 0 ? 29 : 28, 31, 30, 31, 30, 31, 31,
				30, 31, 30, 31 ];
		var str = "", day = 1;
		...
		each.call([ ids['y'], ids['m'] ],
				function(o, i) {
					o.innerHTML = dates[i];
				});
		each.call(ids['calenderDay'].getElementsByTagName("td"),
				function(o, i) {
					addEvent(o, "mouseover", function(e) {
						// o.style.backgroundColor="#66FF99 ";
					});
					...
					addEvent(o, "click", function(e) {
						...
						oInput.value = ids['y'].innerHTML + "-"
								+ ids['m'].innerHTML + "-"
								+ fillzero(o.innerHTML) + " ";
						hide();
					});
				});
	}
	...
	each.call([ ids['yearL'], ids['yearR'], ids['monthL'], ids['monthR'] ], function(
			o, i) {
		addEvent(o, "click", handlers[i]);
	});

	var clicks = [ yClick, mClick ];
	each.call([ ids['y'], ids['m'] ], function(o,
			i) {
...
//	}
	...
	function createOption(type, v) {
		...
		function create(i, i2) {
			while (i >= i2) {
				if (v == i) {
					str2 += '<option value="' + i + '" selected>' + i
							+ '</option>';
				} else {
...
//	}
	...
	function each(handler) {
		...
		for ( var i = 0, len = this.length; i < len; i++) {
			o = typeof this[i] == "string" ? fd.getElementById(this[i])
					: this[i];
			handler(o, i);
		}
	}
	...
	function show(o) {
		...
		var oParentNode = o.parentNode;
		// alert(o+"--"+oParentNode+getPos(o)['x']+"--"+getPos(oParentNode)['x']);
		// alert(getPos(o)['x']-getPos(oParentNode)['x']);
		ff.style.left = getPos(o)['x'] - getPos(oParentNode)['x'] + "px";
		// ff.style.left=getPos(o)['x']+"px";
		oParentNode.style.position = "relative";
		ff.style.top = getPos(o)['y'] - getPos(oParentNode)['y']
				+ o.offsetHeight + "px";
		// ff.style.top=getPos(o)['y']+o.offsetHeight+"px";
	...
	function addEvent(element, event, handler) {
		var element = typeof element == "string" ? d.getElementById(element)
				: element;
