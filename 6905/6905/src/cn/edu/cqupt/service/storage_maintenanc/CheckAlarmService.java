...
public class CheckAlarmService {
	...
	public boolean inspectAlarm(String version)
	{
		...
		if(monthStr.length() == 1)
			monthStr = "0" + monthStr;
		if(logDAO.queryCheckAlarmLog(year, monthStr))//如果这个月已经检查过了，就不提醒，也不执行下面的代码
		{
			flag = false;
		...
		else
		{
			...
			if((month % number) == 0)//这个月底需要提醒
			{
				if(day > (maxDay - aheaddays))//到了该提醒的那几天
				{
					flag = true;
					Log log = new Log();
					log.setOperateType("抽查提醒");
					log.setInspectPerson("");
					log.setMainTainType("");
					log.setOperateTime(now);
					log.setProductId(0);
					log.setRemark("抽查提醒，时间："+ year + "-" + month + "-" + day);
