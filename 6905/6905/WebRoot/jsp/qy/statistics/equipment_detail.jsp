...
<body>
	...
	<div id="right">
		...
		<div class="fare-data" id="fare-data">
			<div class="search_box">
				<form action="EquipmentServlet?operate=equipmentDetail"
					...
					<span>
						<input class="setTem" type="text" name="inYear" id="inYearId"/>
					</span> 
					<span>器材代码</span> 
					<span>
						<input class="setTem" type="text" name="QCBM" />
					</span> 
					<span>器材名称</span> 
					<span>
						<input class="setTem" type="text" name="productName" />
					</span> 
					<span> 
			...
			<div id="equipment_detail-box">
				...
				<table id="equipment_detail">
					<thead>
						<tr>
							<th><input class="setTem" type="checkbox"
								id="checkboxLeader" onclick="chooseAll();"></th>
							<th>序号</th>
							<th>器材代码</th>
							<th>器材名称</th>
							<th>生产单位</th>
		...
		<div class="page-box">
			...
			<span> <a
				href="EquipmentServlet?operate=equipmentDetail&curPageNum=1&pageSize=<%=pageSize%>&inYear=<%=inYear%>&QCBM=<%=QCBM%>&productName=<%=productName%>&keeper=<%=keeper%>">首页</a>
				<a
				href="EquipmentServlet?operate=equipmentDetail&curPageNum=<%=prePageNum %>&pageSize=<%=pageSize %>&inYear=<%=inYear%>&QCBM=<%=QCBM%>&productName=<%=productName%>&keeper=<%=keeper%>">&lt;&lt;</a>
				<span><%=curPageNum%></span> <a
				href="EquipmentServlet?operate=equipmentDetail&curPageNum=<%=nextPageNum %>&pageSize=<%=pageSize %>&productName=<%=inYear%>&QCBM=<%=QCBM%>&productName=<%=productName%>&keeper=<%=keeper%>">&gt;&gt;</a>
				<a
				href="EquipmentServlet?operate=equipmentDetail&curPageNum=<%=totalPageNum %>&pageSize=<%=pageSize %>&inYear=<%=inYear%>&QCBM=<%=QCBM%>&productName=<%=productName%>&keeper=<%=keeper%>">尾页</a>
