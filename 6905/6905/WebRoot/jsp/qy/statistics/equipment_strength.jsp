...
<body>
	...
	<div id="right">
		...
		<div class="subName">
			...
			<a
				href="EquipmentServlet?operate=equipmentCollective&curPageNum=1&pageSize=10">器材实力汇总统计</a>
		</div>
		<!-- 二级标题结束 -->
			...
			<div class="search_box">
				<form
					...
					<span style="margin-left: 10px;">在库年份</span> <span><input
						class="setTem" name="inYear" type="text" id="inYearId" /> </span> <span>器材代码</span>
					<span><input class="setTem" name="QCBM" type="text" /> </span> <span>产品名称</span>
					<span><input class="setTem" name="productName" type="text" /></span>
					<span>产品型号</span> <span><input class="setTem"
						name="productModel" type="text" /> </span> 
						<input class="scan-btn" type="submit" value="查询"> 
						<input type="button" value="导出"
						class="scan-btn" onclick="equipmentCollectiveToWord();">
						<input type="button" value="全部导出"
						class="scan-btn" onclick="allEquipmentCollectiveToWord();">
				</form>
			</div>
			<!--搜索框结束-->
					...
					<div id="equipment_strength-box">
						...
						<table id="equipment_strength">
							...
							<tbody>
								...
								%>
								<tr>
									<td><input type="checkbox" name="checkbox_strength" /></td>
			...
			//用于实力统计在库年份的判断
		      function judgeInYearId(){
		        ...
		        if(b==false) {
		            ...
		            window.wxc.xcConfirm("输入的 ' "+inYearId+" '非法，请重新输入 ！ ","error");
