	...
	<div id="right">
		...
		<div class="fare-data" id="fare-data">
			<div class="search_box">
				<form action="EquipmentServlet?operate=equipmentDetail"
					...
					</span> <span>器材代码</span> <span><input class="setTem" type="text"
						name="QCBM" />
					</span> <span>器材名称</span> <span><input class="setTem" type="text"
						name="productName" />
					</span> 
					<span>代储企业</span>
					...
					<span> <input class="scan-btn" type="submit" value="查询">
						...
						onclick="equipmentDetailToWord();"> 
						<input type="button" value="全部导出"
						class="scan-btn" onclick="allEquipmentDetailToWord();">
						</span>
				</form>
			</div>
		...
		<div class="page-box">
			...
			<span> <a
				href="EquipmentServlet?operate=equipmentDetail&curPageNum=1&pageSize=<%=pageSize %>&productName=<%=productName %>&QCBM=<%=QCBM%>&inYear=<%=inYear%>&keeper=<%=keeper%>">首页</a>
				<a
				href="EquipmentServlet?operate=equipmentDetail&curPageNum=<%=prePageNum %>&pageSize=<%=pageSize %>&productName=<%=productName %>&QCBM=<%=QCBM%>&inYear=<%=inYear%>&keeper=<%=keeper%>">&lt;&lt;</a>
				<span><%=curPageNum %></span> <a
				href="EquipmentServlet?operate=equipmentDetail&curPageNum=<%=nextPageNum %>&pageSize=<%=pageSize %>&productName=<%=productName %>&QCBM=<%=QCBM%>&inYear=<%=inYear%>&keeper=<%=keeper%>">&gt;&gt;</a>
				<a
				href="EquipmentServlet?operate=equipmentDetail&curPageNum=<%=totalPageNum %>&pageSize=<%=pageSize %>&productName=<%=productName %>&QCBM=<%=QCBM%>&inYear=<%=inYear%>&keeper=<%=keeper%>">尾页</a>
			</span>
			...
			<span>每页显示</span> <select name="selectPageSize"
				onchange='selectPageSize(this.value)' id="selectPageSize">
